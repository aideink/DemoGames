...
// Initialize broadcast channel for communication
...
function fetchWithBypass(request, bypassCache) {
    ...
    if (bypassCache) return fetch(request.url, { headers: request.headers, mode: request.mode, credentials: request.credentials, redirect: request.redirect, cache: "no-store" });
